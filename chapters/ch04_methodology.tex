...
\begin{document}
    ...
    \begin{figure}[h]
		\hspace*{-2cm}    
    	\includegraphics[scale=0.55,trim = 1cm 2.50cm 1cm 2.50cm, clip]{chapter4/1_2020.pdf}
    \end{figure}
    ...
    \section{Evaluation}
    Evaluating XAI methods based on their explanation correctness remains a challenge till date. Yeh \etal \cite{yeh2019fidelity} classifies explanation evaluation measures into two categories: objective measures and subjective measures. Objective measures evaluate explanations based on whether a given XAI method satisfies certain properties or axioms, like sensitivity and implementation invariance \cite{sundararajan2017axiomatic}. On the other hand, subjective measures focus on aspects such as explanation usefulness, and the evaluation procedure involves humans. This research work uses subjective measures to evaluate the explanation artifacts produced by the chosen set of XAI methods, when applied to the GestaltMatcher model. This section gives the rationale behind opting for a subjective evaluation procedure and also describes the process in detail.
    \subsection{The Need for Subjective Evaluation}
